...
//  PSExtensions
//
//  Created by PoiSon on 15/10/23.
//  Copyright © 2015年 yerl. All rights reserved.
//

#import "PSDicRecord_private.h"

@interface PSDbAttribute()
@property (nonatomic, nonnull, strong) id<PSDictionaryProtocol> attrs;
@end

@implementation PSDbAttribute{
    PSDbConfig *_config;
}
...
- (void)processContainerWithConfig:(NSString *)config{
    _config = [PSDbKit configForName:config];
    PSAssert(_config, @"can't find config named %@", config);
    id<PSDictionaryProtocol> newAttrs = [_config.containerFactory createDictionary];
    [newAttrs setDictionary:[self.attrs toDictionary]];
    _attrs = newAttrs;
}

- (id<PSDictionaryProtocol>)attrs{
    return _attrs ?: ({_attrs = [self.config.containerFactory createDictionary];});
}

- (PSDbConfig *)config{
    return _config ?: ({_config = [PSDbKit mainConfig] ?: [PSDbKit brokenConfig];});
}
...
#define CONVERT_OC_TYPE_AND_RETURN(type) \
    id value = [self.attrs valueForKey:aKey]; \
    id<PSTypeConvertor> convertor = [PSDbKit convertorForType:[NSString stringWithFormat:@"@\"%s\"", #type]]; \
...
#define CONVERT_TYPE_AND_RETURN(type) \
    id value = [self.attrs valueForKey:aKey]; \
    id<PSTypeConvertor> convertor = [PSDbKit convertorForType:@(@encode(type))]; \
    type result; \
    [convertor getBuffer:&result fromObject:value]; \
    return result;
