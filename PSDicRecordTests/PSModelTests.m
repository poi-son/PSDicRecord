...
/** 测试分页. */
- (void)testPage{
    PSPage<Student *> *page = [[Student dao] paginate:1 size:10 withSelect:@"select *" where:@"from Student where 1 = ?", @(1)];
...
- (void)testPerformanceInsert{
    [self measureBlock:^{
        [[PSDb main] tx:^BOOL(PSDb * _Nonnull db) {
